...
parameters {
	...
	real<lower=betakp_lower,upper=betakp_upper> betakp; // fixed effects, knot point: fixed knot for all patients but treated as an unknown parameter (bounding specified to help convergence)
...
transformed parameters {	
	...
	for (j in 1:N) {
		if (age[j] < betakp) 
			y_mu[j] <- alpha[id[j],1] + alpha[id[j],2] * (age[j] - betakp);
		else 	
			y_mu[j] <- alpha[id[j],1] + alpha[id[j],3] * (age[j] - betakp);			
	}	
}
