...
parameters {
	...
	vector<lower=0>[3] u_sd;							// level 2 error sd (sds of the random effects u[j]), intercept and slopes
	real<lower=0> ukp_sd;								// level 2 error sd, knot point
	real<lower=0> y_sd;									// level 1 error sd
	vector[3] uRaw[Npat];								// working parameters for random effects (level 2) errors
	real ukpRaw[Npat];									// working parameters for random effects (level 2) errors, knot point
	cholesky_factor_corr[3] L_u_Corr;					// cholesky factor for the random effects correlation matrix
}

transformed parameters {	
	vector[3] u[Npat];									// random effects (level 2) errors, intercept and slopes
	real ukp[Npat];										// random effects (level 2) errors, knot point
	...
	// matts trick
	for (i in 1:Npat) {
		u[i] <- u_sd .* (L_u_Corr * uRaw[i]); 	// equivalent to: diag_matrix(u_sd) * L_u_Corr * uRaw[i];
		ukp[i] <- ukp_sd * ukpRaw[i];
	}
	
	for (i in 1:Npat) {
		for (k in 1:3) alpha[i,k] <- beta[k] + u[i,k];
		alpha[i,4] <- betakp + ukp[i];
	}
...
model {
	...
	ukp_sd ~ cauchy(0,5);								// prior: random effect sd, knot point
